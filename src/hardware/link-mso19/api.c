 ...
 * Copyright (C) 2020 Paul Kasemir <paul.kasemir@gmail.com>
 *
...
static void mso_update_trigger_pos(struct dev_context *devc)
{
	...
	uint16_t sign_bit = TRIG_POS_IS_POSATIVE;
	if (pos < 0) {
		pos = -pos;
...
static GSList* scan_handle_port(GSList *devices, struct sp_port *port)
{
	...
	devc->logic_threshold = 5; // 3.3V/5V
	...
	const struct sr_dev_inst *sdi, const struct sr_channel_group *cg)
{
	...
	case SR_CONF_TRIGGER_SLOPE:
		if (cg_is_analog(cg)) {
			if ((idx = std_str_idx(data, ARRAY_AND_SIZE(dso_trigger_slopes))) < 0)
				return SR_ERR_ARG;
			devc->dso_trigger_slope = idx;
		} else if (cg_is_digital(cg)) {
			if ((idx = std_str_idx(data, ARRAY_AND_SIZE(la_trigger_slopes))) < 0)
				return SR_ERR_ARG;
	...
	case SR_CONF_HORIZ_TRIGGERPOS:
		...
		break;
	case SR_CONF_COUPLING:
		if (!cg_is_analog(cg) || (idx = std_str_idx(data, ARRAY_AND_SIZE(coupling))) < 0)
			return SR_ERR_ARG;
